...
local function do_authentication(conf)
  ...
  if token_type ~= "string" then
    if token_type == "nil" then
      return false, { status = 401, message="UNAUTHORIZED", description = "Authentication failed for lack of authentication credentials." }
    elseif token_type == "table" then
      return false, { status = 401, message="UNAUTHORIZED", description = "Multiple tokens provided." }
    else
      return false, { status = 401, message="UNAUTHORIZED", description = "Unrecognizable token." }
    end
  end
  ...
  if err then
    return false, { status = 401, message="UNAUTHORIZED", description = "Bad token; " .. tostring(err) }
  end
  ...
  if not jwt_secret_key then
    return false, { status = 401, message="UNAUTHORIZED",  description = "No mandatory '" .. conf.key_claim_name .. "' in claims." }
  elseif jwt_secret_key == "" then
    return false, { status = 401, message="UNAUTHORIZED", description = "Invalid '" .. conf.key_claim_name .. "' in claims." }
  end
  ...
  if not jwt_secret then
    return false, { status = 401, message="UNAUTHORIZED", description = "No credentials found for given '" .. conf.key_claim_name .. "'" }
  end
  ...
  if jwt.header.alg ~= algorithm then
    return false, { status = 401, message="UNAUTHORIZED", description = "Invalid algorithm." }
  end
  ...
  if not jwt_secret_value then
    return false, { status = 401, message="UNAUTHORIZED", description = "Invalid key/secret." }
  end

  -- Now verify the JWT signature
  if not jwt:verify_signature(jwt_secret_value) then
    return false, { status = 401, message="UNAUTHORIZED", description = "Invalid signature." }
  end

  -- Verify the JWT registered claims
  local ok_claims, errors = jwt:verify_registered_claims(conf.claims_to_verify)
  if not ok_claims then
    return false, { status = 401, errors = errors['exp'] }
  end
  ...
  if not consumer then
    return false, {
      ...
      description = fmt("Could not find consumer for '%s=%s'", conf.key_claim_name, jwt_secret_key)
    }
  end
...
function JwtHandler:access(conf)
  ...
  if not ok then
    if conf.anonymous then
      ...
      return kong.response.exit(err.status, { code = err.status, message = err.message and err.message or err.errors})
    end
  end
end
