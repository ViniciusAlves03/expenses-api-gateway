  ...
  if not consumer_id then
    ...
    if not authenticated_groups then
      if kong.client.get_credential() then
        return kong.response.exit(403, { code = 403, message="FORBIDDEN", description = "Authorization failed due to insufficient permissions." })
      end

      return kong.response.exit(401, { code = 401, message="UNAUTHORIZED", description = "Authentication failed for lack of authentication credentials." })
    end
